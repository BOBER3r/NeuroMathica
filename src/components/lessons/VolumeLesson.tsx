...
function HookStage({ onComplete }: { onComplete: () => void }) {
  return <VideoHook src="/videos/VolumeHook.webm" onComplete={onComplete} />;
      ...
      <svg viewBox={`0 0 ${svgW} ${svgH}`} className="w-full max-w-md" aria-label="3D solids filling with cubes">
        ...
        {phase >= 1 && (
          <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }}>
            <rect x={20} y={60} width={80} height={100} fill={COLORS.prismFill} stroke={COLORS.prism} strokeWidth={2} rx={2} />
            {/* Grid lines to show unit cubes */}
            {[1, 2, 3].map((r) => (
              <line key={`h${r}`} x1={20} y1={60 + r * 25} x2={100} y2={60 + r * 25}
                stroke={COLORS.prism} strokeWidth={0.5} opacity={0.5} />
            ))}
            {[1, 2, 3].map((c) => (
              <line key={`v${c}`} x1={20 + c * 20} y1={60} x2={20 + c * 20} y2={160}
                stroke={COLORS.prism} strokeWidth={0.5} opacity={0.5} />
            ))}
            <text x={60} y={185} textAnchor={"middle" as const} fill={COLORS.prism} fontSize={11} fontWeight={600}>
              Prism
            </text>
          </motion.g>
        )}
        ...
        {phase >= 2 && (
          <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }}>
            <ellipse cx={170} cy={70} rx={35} ry={10} fill={COLORS.cylinderFill} stroke={COLORS.cylinder} strokeWidth={2} />
            <line x1={135} y1={70} x2={135} y2={155} stroke={COLORS.cylinder} strokeWidth={2} />
            <line x1={205} y1={70} x2={205} y2={155} stroke={COLORS.cylinder} strokeWidth={2} />
            <ellipse cx={170} cy={155} rx={35} ry={10} fill={COLORS.cylinderFill} stroke={COLORS.cylinder} strokeWidth={2} />
            <text x={170} y={185} textAnchor={"middle" as const} fill={COLORS.cylinder} fontSize={11} fontWeight={600}>
              Cylinder
            </text>
          </motion.g>
        )}
        ...
        {phase >= 3 && (
          <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }}>
            <line x1={280} y1={60} x2={250} y2={155} stroke={COLORS.cone} strokeWidth={2} />
            <line x1={280} y1={60} x2={310} y2={155} stroke={COLORS.cone} strokeWidth={2} />
            <ellipse cx={280} cy={155} rx={30} ry={8} fill={COLORS.coneFill} stroke={COLORS.cone} strokeWidth={2} />
            <text x={280} y={185} textAnchor={"middle" as const} fill={COLORS.cone} fontSize={11} fontWeight={600}>
              Cone
            </text>
          </motion.g>
        )}
...
function SpatialStage({ onComplete }: { onComplete: () => void }) {
  ...
  const solidColor = solid === "prism" ? COLORS.prism : solid === "cylinder" ? COLORS.cylinder
    : solid === "cone" ? COLORS.cone : COLORS.sphere;
  const solidFill = solid === "prism" ? COLORS.prismFill : solid === "cylinder" ? COLORS.cylinderFill
    : solid === "cone" ? COLORS.coneFill : COLORS.sphereFill;
    ...
    <section className="flex flex-1 flex-col items-center px-4 pt-4"
      ...
      <div className="flex gap-1 justify-center mb-3 flex-wrap">
        {(["prism", "cylinder", "cone", "sphere"] as const).map((s) => {
          const c = s === "prism" ? COLORS.prism : s === "cylinder" ? COLORS.cylinder
            : s === "cone" ? COLORS.cone : COLORS.sphere;
          return (
            <button key={s} onClick={() => { setSolid(s); interact(); }}
              ...
              style={{ backgroundColor: solid === s ? c : COLORS.bgSurface, color: COLORS.textPrimary }}>
      ...
      <div className="w-full max-w-xs space-y-2 mb-2">
        <div className="flex items-center gap-3">
          <span className="text-xs w-16" style={{ color: COLORS.textMuted }}>
        ...
        {solid === "prism" && (
          <div className="flex items-center gap-3">
            <span className="text-xs w-16" style={{ color: COLORS.textMuted }}>Width</span>
        ...
        {solid !== "sphere" && (
          <div className="flex items-center gap-3">
            <span className="text-xs w-16" style={{ color: COLORS.textMuted }}>Height</span>
      ...
      {/* Formula */}
      <div className="rounded-xl p-3 w-full max-w-xs text-center" style={{ backgroundColor: COLORS.bgSurface }}>
  ...
  return (
    <section className="flex flex-1 flex-col items-center justify-center px-4"
      ...
      <svg viewBox="0 0 260 140" className="w-full max-w-[260px] mb-6">
          ...
          <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            {[0, 1, 2, 3, 4].map((i) => (
              <motion.rect key={i} x={70} y={20 + i * 20} width={120} height={18}
                fill={COLORS.prismFill} stroke={COLORS.prism} strokeWidth={1} rx={2}
                initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
                transition={{ delay: i * 0.15 }} />
            ))}
            <text x={130} y={135} textAnchor={"middle" as const} fill={COLORS.prism} fontSize={13} fontWeight={600}>
              Base {"\u00D7"} height
            </text>
          </motion.g>
        )}
        {step === 1 && (
          <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <ellipse cx={130} cy={30} rx={50} ry={12} fill={COLORS.cylinderFill} stroke={COLORS.cylinder} strokeWidth={2} />
            <line x1={80} y1={30} x2={80} y2={110} stroke={COLORS.cylinder} strokeWidth={2} />
            <line x1={180} y1={30} x2={180} y2={110} stroke={COLORS.cylinder} strokeWidth={2} />
            <ellipse cx={130} cy={110} rx={50} ry={12} fill={COLORS.cylinderFill} stroke={COLORS.cylinder} strokeWidth={2} />
            <text x={130} y={135} textAnchor={"middle" as const} fill={COLORS.cylinder} fontSize={13} fontWeight={600}>
              {"\u03C0"}r{"\u00B2"} {"\u00D7"} h
            </text>
          </motion.g>
        )}
        {step === 2 && (
          <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            {/* Cylinder ghost */}
            <ellipse cx={130} cy={110} rx={50} ry={12} fill="none" stroke={COLORS.cylinder} strokeWidth={1}
              strokeDasharray="4,3" opacity={0.4} />
            <line x1={80} y1={30} x2={80} y2={110} stroke={COLORS.cylinder} strokeWidth={1}
              strokeDasharray="4,3" opacity={0.4} />
            <line x1={180} y1={30} x2={180} y2={110} stroke={COLORS.cylinder} strokeWidth={1}
              strokeDasharray="4,3" opacity={0.4} />
            {/* Cone */}
            <line x1={130} y1={20} x2={80} y2={110} stroke={COLORS.cone} strokeWidth={2} />
            <line x1={130} y1={20} x2={180} y2={110} stroke={COLORS.cone} strokeWidth={2} />
            <ellipse cx={130} cy={110} rx={50} ry={12} fill={COLORS.coneFill} stroke={COLORS.cone} strokeWidth={2} />
            <text x={130} y={135} textAnchor={"middle" as const} fill={COLORS.cone} fontSize={13} fontWeight={600}>
              (1/3) {"\u00D7"} cylinder
            </text>
          </motion.g>
        )}
...
function SymbolBridgeStage({ onComplete }: { onComplete: () => void }) {
  ...
  const notations = [
    { formula: "V = lwh", desc: "Rectangular prism: length \u00D7 width \u00D7 height", color: COLORS.prism },
    { formula: "V = \u03C0r\u00B2h", desc: "Cylinder: circle area times height", color: COLORS.cylinder },
    { formula: "V = (1/3)\u03C0r\u00B2h", desc: "Cone: one-third of a cylinder", color: COLORS.cone },
    { formula: "V = (4/3)\u03C0r\u00B3", desc: "Sphere: four-thirds pi r-cubed", color: COLORS.sphere },
  ];
    ...
    <div className="flex min-h-screen flex-col" style={{ backgroundColor: COLORS.bgPrimary }}>
      <div className="sticky top-0 z-10 backdrop-blur-sm px-4 py-2"
        ...
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs font-medium" style={{ color: COLORS.textMuted }}>GE-4.7 Volume</span>
